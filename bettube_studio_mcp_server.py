...
def _transport_from_env() -> str:
    return str(os.getenv("BETTUBE_STUDIO_MCP_TRANSPORT") or "stdio").strip().lower()


def _host_from_env() -> str:
    return str(os.getenv("BETTUBE_STUDIO_MCP_HOST") or "127.0.0.1").strip()


def _port_from_env() -> int:
    return int(os.getenv("BETTUBE_STUDIO_MCP_PORT") or "8765")


def _path_from_env() -> str:
    value = str(os.getenv("BETTUBE_STUDIO_MCP_HTTP_PATH") or "/mcp").strip()
