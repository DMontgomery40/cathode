...
def _write_wrapper_script(path: Path) -> None:
    ...
    script = textwrap.dedent(
        """
            ...
            import bettube_studio_mcp_server as bettube_studio
            ...
            def fake_create_make_video_job(**kwargs):
                ...
                project_dir = bettube_studio.PROJECTS_DIR / project_name
                return {
                    "status": "queued",
                    "job_id": "job-123",
            ...
            def fake_get_job_status(job_id, project_name=None):
                ...
                return {
                    ...
                    "project_dir": str(bettube_studio.PROJECTS_DIR / "demo_project"),
                    "current_stage": "done",
                    "retryable": False,
                    "suggestion": "",
                    "requested_stage": "render",
                    "pid": None,
                    "result": {"video_path": str(bettube_studio.PROJECTS_DIR / "demo_project" / "demo_project.mp4")},
                    "error": None,
                }
            ...
            def fake_create_rerun_stage_job(**kwargs):
                return {
                    ...
                    "job_id": "job-rerun",
                    "project_name": kwargs["project_name"],
                    "project_dir": str(bettube_studio.PROJECTS_DIR / kwargs["project_name"]),
                    "current_stage": "queued",
                    "retryable": False,
                    "suggestion": "",
            ...
            bettube_studio.choose_llm_provider = fake_choose_llm_provider
            bettube_studio.create_make_video_job = fake_create_make_video_job
            bettube_studio.get_job_status = fake_get_job_status
            bettube_studio.cancel_job = fake_cancel_job
            bettube_studio.create_rerun_stage_job = fake_create_rerun_stage_job
            bettube_studio.load_plan = fake_load_plan
            bettube_studio.collect_project_artifacts = fake_collect_project_artifacts
            bettube_studio.list_projects = fake_list_projects
            bettube_studio.list_project_jobs = fake_list_project_jobs

            bettube_studio.PROJECTS_DIR.mkdir(exist_ok=True)
            server = bettube_studio.build_server()
            server.run(transport=os.environ.get("BETTUBE_STUDIO_TEST_TRANSPORT", "stdio"))
            """
    )
...
def test_streamable_http_server_supports_elicitation(tmp_path):
    ...
    env = dict(
        os.environ,
        BETTUBE_STUDIO_TEST_TRANSPORT="streamable-http",
        BETTUBE_STUDIO_MCP_TRANSPORT="streamable-http",
        BETTUBE_STUDIO_MCP_PORT=str(port),
        BETTUBE_STUDIO_MCP_HOST="127.0.0.1",
