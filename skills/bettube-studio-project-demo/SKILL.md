...
2. Bootstrap betTube Studio with `scripts/bootstrap_bettube_studio.py`.
...
9. Build the betTube Studio `make_video` payload with `scripts/prepare_bettube_studio_handoff.py`.
...
- Run `python3 scripts/bootstrap_bettube_studio.py --repo-path /path/to/bettube-studio` when betTube Studio is already checked out locally.
- Run `python3 scripts/bootstrap_bettube_studio.py` only when no usable checkout exists.
...
- Run `python3 scripts/prepare_bettube_studio_handoff.py` with the bundle manifest, review report, target repo path, and demo brief fields.
...
- `scripts/bootstrap_bettube_studio.py`: prepare or reuse a betTube Studio checkout
...
- `scripts/prepare_bettube_studio_handoff.py`: build the `make_video` payload from the reviewed bundle
